...
// ✅ Include the auto-absent script but do NOT call it on every page load (too slow)
// We'll provide a manual trigger button so admin can run it when needed.
include 'mark_absent_students.php';

// Get all active sections for filters
try {
    $sectionsQuery = $conn->query("SELECT DISTINCT year_section FROM student_tbl WHERE status = 'Active' AND year_section IS NOT NULL ORDER BY year_section");
    $sections = $sectionsQuery->fetchAll(PDO::FETCH_COLUMN);
...
if (strpos($profileImage, 'http') === 0) {
    // It's a full URL (like from UI Avatars or Supabase)
    $profileImageUrl = $profileImage;
} else if (!empty($profileImage)) {
    // It's a filename stored in Supabase
...
#searchResultsDropdown .dropdown-item:focus,
#searchResultsDropdown .dropdown-item.active {
    color: #16181b;
 ...
 <!-- Content Row -->
                    <div class="row g-4">
                        ...
                        <div class="col-xl-4 col-md-6 mb-4">
                            <div class="card shadow h-100 py-3 px-2" style="border-left: 8px solid #4e73df;">
                                <div class="card-body">
                                    <div class="row align-items-center">
                                        <div class="col mr-3">
                                            ...
                                            <div class="mb-0 text-gray-800" style="font-size: 2rem; font-weight: 700;">
                                                <?php 
                                                    $stmt = $conn->query("SELECT COUNT(*) FROM staff_tbl WHERE user_role = 'Teacher'");
    ...
    <div class="card shadow h-100">
        <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #1cc88a;">
            ...
            <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                <select id="trendGradeFilter" class="form-control form-control-sm" style="width: 120px;">
                        ...
                        <div class="col-xl-4 col-lg-6 mb-3">
                            <div class="card shadow h-100">
                                <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #4e73df;">
                                    ...
                                    <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                                        <select id="todayStatusFilter" class="form-control form-control-sm" style="width: 90px;">
                                            <option value="All">All Grades</option>
                                             <?php foreach ($sections as $section): ?>
        <option><?= htmlspecialchars($section) ?></option>
    <?php endforeach; ?>
                                        </select>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-xl-4 col-lg-6 mb-3">
                            <div class="card shadow h-100">
                                <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #36b9cc;">
                                    <h6 class="m-0 font-weight-bold text-white">Gender Distribution</h6>
                                    <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                                        <select id="genderGradeFilter" class="form-control form-control-sm" style="width: 120px;">
                                            <option value="All">All Grades</option>
                                             <?php foreach ($sections as $section): ?>
        <option><?= htmlspecialchars($section) ?></option>
    <?php endforeach; ?>
...
<div class="col-xl-8 col-lg-6 mb-3">
    <div class="card shadow h-100">
        <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #4e73df;">
            <h6 class="m-0 font-weight-bold text-white">Attendance Comparison by Year & Section</h6>
            <!-- Remove period filter, show only today -->
            <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                <span class="text-white">Today</span>
            </div>
        </div>
        <div class="card-body position-relative">
            <div class="chart-loading" id="sectionChartLoading">
                <div class="loading-spinner"></div>
                <span>Loading data...</span>
            </div>
            <div class="chart-bar">
                <canvas id="sectionAttendanceChart" height="300"></canvas>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="row mt-4">
                       <!-- Top Sections with Highest Attendance -->
<div class="col-xl-7 col-lg-12 mb-3">
    <div class="card shadow h-100">
        <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #1cc88a;">
            <h6 class="m-0 font-weight-bold text-white">Top Performing Sections</h6>
            <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                <span class="text-white">Today</span>
            </div>
        </div>
        <div class="card-body position-relative">
            <div class="chart-loading" id="topSectionsLoading">
                <div class="loading-spinner"></div>
                <span>Loading data...</span>
            </div>
            <div class="chart-bar">
                <canvas id="topSectionsChart" height="300"></canvas>
            </div>
        </div>
    </div>
</div>
...
<div class="col-xl-5 col-lg-6 mb-3">
    <div class="card shadow h-100">
        <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #36b9cc;">
            ...
            <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                <!-- Only show grade filter, remove period filter -->
                <select id="sessionGradeFilter" class="form-control form-control-sm" style="width: 120px;">
                    <option value="All">All Grades</option>
                    <?php foreach ($sections as $section): ?>
                        <option><?= htmlspecialchars($section) ?></option>
                    <?php endforeach; ?>
                        ...
                        <div class="col-xl-12 col-lg-12">
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-header py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between" style="background-color: #4e73df;">
                                    ...
                                    <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                                        <select id="tableGradeFilter" class="form-control form-control-sm" style="width: 150px;color: #4e73df;">
                                            <option value="All">All Grades</option>
                                         <?php foreach ($sections as $section): ?>
        <option><?= htmlspecialchars($section) ?></option>
    <?php endforeach; ?>
 ...
 <!-- Profile Modal -->
   ...
   <script>
    ...
    <script>
        ...
        let attendanceTrendChart, attendanceTodayChart, genderDistributionChart;
        let sectionAttendanceChart, topSectionsChart, amSessionChart, pmSessionChart;
        ...
        $(document).ready(function() {
            ...
            $('.session-toggle').click(function() {
                ...
                updateTodaysAttendancePercentage();
            });
        });
        ...
        function loadInitialData() {
            ...
            loadSectionAttendanceChart();
            loadTopSectionsChart();
            loadSessionCharts();
        ...
        function setupFilterListeners() {
            ...
            $('#trendGradeFilter').change(loadAttendanceTrendChart);
            
            // Attendance Today Chart
            $('#todayStatusFilter, #todaySessionFilter').change(loadAttendanceTodayChart);
            
            // Gender Distribution Chart
            $('#genderGradeFilter').change(loadGenderDistributionChart);
            
            // Section Attendance Chart
            $('#attendanceSectionFilter').change(loadSectionAttendanceChart);
            
            // Top Sections Chart
            $('#topSectionFilter').change(loadTopSectionsChart);
            
            // Session Charts
            $('#sessionTypeFilter, #sessionGradeFilter').change(loadSessionCharts);
            
            // Today's Attendance Table
$('#tableGradeFilter').change(function() {
    loadTodaysAttendanceTable(true); // Reset to page 1 when filter changes
});
        }

        function updateTodaysAttendancePercentage() {
            const session = $('.session-toggle.active').data('session').toUpperCase();
            $.ajax({
                url: 'ajax/get_todays_attendance_percentage.php',
                type: 'GET',
        ...
        function loadAttendanceTrendChart() {
            ...
            const grade = $('#trendGradeFilter').val();
            
            $.ajax({
                url: 'ajax/get_attendance_trend.php',
                type: 'GET',
                data: { grade: grade },
                dataType: 'json',
   ...
   function loadAttendanceTodayChart() {
    ...
    const grade = $('#todayStatusFilter').val();
    const session = $('#todaySessionFilter').val();
    
    $.ajax({
        url: 'ajax/get_attendance_today_distribution.php',
        type: 'GET',
...
function updateTodayChart(data) {
    ...
    attendanceTodayChart = new Chart(ctx, {
        ...
        function loadGenderDistributionChart() {
            ...
            const grade = $('#genderGradeFilter').val();
            
            $.ajax({
                url: 'ajax/get_gender_distribution.php',
                type: 'GET',
                data: { grade: grade },
                dataType: 'json',
                success: function(data) {
    ...
    genderDistributionChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    display: false // Disable the built-in legend since we have a custom one
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            const label = context.datasetIndex === 0 ? 
                                (context.dataIndex === 0 ? 'Male' : 'Female') : '';
      ...
      function loadSectionAttendanceChart() {
    showLoading('sectionChartLoading');
    // Always use 'today' for period
    $.ajax({
        url: 'ajax/get_attendance_by_section.php',
        type: 'GET',
        data: { period: 'today' },
        dataType: 'json',
        success: function(data) {
            updateSectionChart(data);
            hideLoading('sectionChartLoading');
        },
        error: function() {
            hideLoading('sectionChartLoading');
            alert('Error loading section attendance data');
        }
    });
}

        function updateSectionChart(data) {
            const ctx = document.getElementById('sectionAttendanceChart').getContext('2d');
            
            if (sectionAttendanceChart) {
                sectionAttendanceChart.destroy();
            }
            
            sectionAttendanceChart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: data.labels,
                    datasets: [{
                        label: 'AM Attendance',
                        data: data.am_percentages,
                        backgroundColor: '#4e73df',
                        borderColor: '#4e73df',
                        borderWidth: 1
                    }, {
                        label: 'PM Attendance',
                        data: data.pm_percentages,
       ...
       function loadTopSectionsChart() {
    showLoading('topSectionsLoading');
    $.ajax({
        url: 'ajax/get_top_sections.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            updateTopSectionsChart(data);
            hideLoading('topSectionsLoading');
        },
        error: function() {
            hideLoading('topSectionsLoading');
            alert('Error loading top sections data');
        }
    });
}

        function updateTopSectionsChart(data) {
    ...
    const grade = $('#sessionGradeFilter').val();
    $.ajax({
        url: 'ajax/get_attendance_by_session.php',
        type: 'GET',
        data: { period: 'today', grade: grade },
        dataType: 'json',
        success: function(data) {
    ...
    const totalAm = data.am.present + data.am.absent + data.am.not_marked;
    const totalPm = data.pm.present + data.pm.absent + data.pm.not_marked;
    
    // Clear any previous messages and show loading container
    container.innerHTML = '';
    container.style.display = 'none';
    
    // AM Session Chart
    const amContainer = chartContainer.querySelector('#amSessionChart').parentElement;
    const amCanvas = chartContainer.querySelector('#amSessionChart');
    
    // Clear any existing messages in AM container
    const existingAmMessages = amContainer.querySelectorAll('.no-data-message');
    existingAmMessages.forEach(msg => msg.remove());
    
    if (totalAm === 0) {
        amCanvas.style.display = 'none';
        const noDataDiv = document.createElement('div');
        noDataDiv.className = 'no-data-message text-center py-4 mt-5';
        noDataDiv.innerHTML = `
            <i class="fas fa-info-circle text-muted"></i>
            <p class="mt-2 text-muted">No AM attendance recorded</p>
        `;
        amContainer.appendChild(noDataDiv);
    } else {
        ...
        // Compute counts and percentages for AM
        const amCounts = {
            present: data.am.present || 0,
            absent: data.am.absent || 0,
            not_marked: data.am.not_marked || 0
        };
        const amTotal = amCounts.present + amCounts.absent + amCounts.not_marked;
        const amPercent = amTotal > 0 ? [
            Math.round((amCounts.present / amTotal) * 100),
            Math.round((amCounts.absent / amTotal) * 100),
            Math.round((amCounts.not_marked / amTotal) * 100)
        ] : [0,0,0];

        amSessionChart = new Chart(amCtx, {
            type: 'doughnut',
            data: {
                labels: ['Present', 'Absent', 'Not Marked'],
                datasets: [{
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                const label = context.label || '';
                                const value = context.parsed || 0; // percentage
                                let count = 0;
                                switch(label) {
                                    case 'Present': count = amCounts.present; break;
                                    case 'Absent': count = amCounts.absent; break;
                                    case 'Not Marked': count = amCounts.not_marked; break;
                                }
                                return [
                                    ` ${label}: ${value}%`,
                                    `${count} students`
    ...
    // PM Session Chart
    const pmContainer = chartContainer.querySelector('#pmSessionChart').parentElement;
    const pmCanvas = chartContainer.querySelector('#pmSessionChart');
    
    // Clear any existing messages in PM container
    const existingPmMessages = pmContainer.querySelectorAll('.no-data-message');
    existingPmMessages.forEach(msg => msg.remove());
    ...
    if (totalPm === 0) {
        ...
        noDataDiv.innerHTML = `
            <i class="fas fa-info-circle text-muted"></i>
            <p class="mt-2 text-muted">No PM attendance recorded</p>
        `;
        pmContainer.appendChild(noDataDiv);
    } else {
        ...
        // Compute counts and percentages for PM
        const pmCounts = {
            present: data.pm.present || 0,
            absent: data.pm.absent || 0,
            not_marked: data.pm.not_marked || 0
        };
        const pmTotal = pmCounts.present + pmCounts.absent + pmCounts.not_marked;
        const pmPercent = pmTotal > 0 ? [
            Math.round((pmCounts.present / pmTotal) * 100),
            Math.round((pmCounts.absent / pmTotal) * 100),
            Math.round((pmCounts.not_marked / pmTotal) * 100)
        ] : [0,0,0];

        pmSessionChart = new Chart(pmCtx, {
            type: 'doughnut',
            data: {
                labels: ['Present', 'Absent', 'Not Marked'],
                datasets: [{
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                let count = 0;
                                switch(label) {
                                    case 'Present': count = pmCounts.present; break;
                                    case 'Absent': count = pmCounts.absent; break;
                                    case 'Not Marked': count = pmCounts.not_marked; break;
                                }
                                return [
                                    ` ${label}: ${value}%`,
                                    `${count} students`
...
function loadTodaysAttendanceTable(resetPage = true) {
    ...
    const grade = $('#tableGradeFilter').val();
    
    $.ajax({
        url: 'ajax/get_todays_logged_attendance.php',
        type: 'GET',
        data: { 
            grade: grade,
            page: currentPage,
...
// Update the populateAttendanceTable function
...
// Add this to your setupFilterListeners function
...
// No persistent status text is used any more.
    </script>
