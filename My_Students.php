...
$user = $_SESSION['user'];
$teacherId = $user['id'];
$advisorySection = $user['advisory_section'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addStudent'])) {
    ...
    $gender = $_POST['gender'];
    $year_section = $advisorySection;
    $parent_number = $_POST['parent_number'];
    $teacher_id = $teacherId;
    $date_enrolled = $_POST['date_enrolled'];

    // Check if LRN already exists
    $check = $conn->prepare("SELECT 1 FROM student_tbl WHERE lrn = :lrn");
    $check->execute([':lrn' => $lrn]);
    if ($check->fetch()) {
        $_SESSION['error'] = "Student with LRN $lrn already exists.";
    } else {
        $stmt = $conn->prepare("INSERT INTO student_tbl (lrn, stud_name, gender, year_section, parent_number, assigned_teacher_id, date_enrolled) VALUES (:lrn, :stud_name, :gender, :year_section, :parent_number, :assigned_teacher_id, :date_enrolled)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editStudent'])) {
    ...
    $year_section = $advisorySection;
    $parent_number = $_POST['edit_parent_number'];
    $teacher_id = $teacherId;
    $date_enrolled = $_POST['edit_date_enrolled'];

    $stmt = $conn->prepare("UPDATE student_tbl SET lrn=:lrn, stud_name=:stud_name, gender=:gender, year_section=:year_section, parent_number=:parent_number, assigned_teacher_id=:assigned_teacher_id, date_enrolled=:date_enrolled WHERE student_id=:student_id");
    $success = $stmt->execute([
        ':lrn' => $lrn,
        ':stud_name' => $name,
        ':gender' => $gender,
...
// Handle Archive/Restore Student
if (isset($_GET['archive'])) {
    $student_id = $_GET['archive'];
    $status = $_GET['status'];
...
// Fetch active students for this teacher's section
$activeStudents = [];
$activeQuery = $conn->prepare("SELECT * FROM student_tbl WHERE status='Active' AND year_section = :year_section ORDER BY stud_name ASC");
$activeQuery->execute([':year_section' => $advisorySection]);
while ($row = $activeQuery->fetch()) {
    $activeStudents[] = $row;
}

// Fetch archived students for this teacher's section
$archivedStudents = [];
$archivedQuery = $conn->prepare("SELECT * FROM student_tbl WHERE status='Archived' AND year_section = :year_section ORDER BY stud_name ASC");
$archivedQuery->execute([':year_section' => $advisorySection]);
while ($row = $archivedQuery->fetch()) {
...
if (isset($_POST['export_excel'])) {
    ...
    $query = "SELECT lrn, stud_name, gender, year_section, parent_number FROM student_tbl WHERE year_section = :year_section";
    $params = [':year_section' => $advisorySection];
    ...
    fputcsv($output, ['LRN', 'Full Name', 'Gender', 'Year & Section', 'Parent Number']);
...
// Handle Barcode Export
if (isset($_POST['export_barcode'])) {
    $gender = $_POST['gender'] ?? '';
    $status = $_POST['status'] ?? 'Active';
    $per_page = (int)($_POST['per_page'] ?? 8);

    $query = "SELECT lrn, stud_name FROM student_tbl WHERE year_section = :year_section";
    $params = [':year_section' => $advisorySection];
    ...
    <head>
        ...
        <style>
            ...
            @media print {
                ...
                .barcode-print-container { padding: 0; }
...
<div class="d-flex align-items-center justify-content-center flex-column flex-md-row text-center text-md-left px-3 mb-3 pt-2" id="adminProfile">
  ...
  <div class="d-flex align-items-center">
    ...
    <h2 class="fw-bolder mb-0" style="color: rgb(11, 104, 245); font-size: 1.5rem; font-weight: 800;">My Students</h2>
  </div>
</div>
                                     </div>
                    ...
                    <form id="studentForm" method="POST" action="My_Students.php">
                       <input type="hidden" name="addStudent" value="1">
                        ...
                        <div class="form-row">
                            ...
                            <!-- Year & Section -->
                            <div class="form-group col-md-6">
                                  <label class="font-weight-bold required">Year & Section</label>
                                    <input type="text" class="form-control" value="<?= htmlspecialchars($advisorySection) ?>" readonly>
                                <input type="hidden" name="year_section" value="<?= htmlspecialchars($advisorySection) ?>">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <!-- Assign to Teacher -->
                             <div class="form-group col-md-6">
                                <label class="font-weight-bold required">Assign to Teacher</label>
                                <input type="text" class="form-control" value="<?= htmlspecialchars($user['fullname']) ?>" readonly>
                                <input type="hidden" name="assigned_teacher_id" value="<?= $teacherId ?>">
                            </div>
...
<div class="form-group col-md-6">
    ...
    <input type="date" class="form-control" id="dateEnrolledInput" name="date_enrolled" required>
                ...
                <div class="modal-footer">
                   ...
                   <button type="submit" class="btn btn-purple" form="studentForm" id="saveStudentBtn">
    <span class="btn-text"><i class="fas fa-save mr-1"></i> Save Student</span>
</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="editStudentModal" tabindex="-1" role="dialog" aria-labelledby="editStudentModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <!-- Modal Body -->
                <div class="modal-body">
                    <form id="editStudentForm" method="POST" action="My_Students.php">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                <label class="font-weight-bold required">Year & Section</label>
                                <input type="text" class="form-control" value="<?= htmlspecialchars($advisorySection) ?>" readonly>
                                <input type="hidden" name="edit_year_section" value="<?= htmlspecialchars($advisorySection) ?>">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <!-- Assign to Teacher (Fixed) -->
                            <div class="form-group col-md-6">
                                <label class="font-weight-bold required">Assign to Teacher</label>
                                <input type="text" class="form-control" value="<?= htmlspecialchars($user['fullname']) ?>" readonly>
                                <input type="hidden" name="edit_assigned_teacher_id" value="<?= $teacherId ?>">
                            </div>
                ...
                <div class="modal-footer">
                   ...
                   <button type="submit" class="btn btn-purple" form="editStudentForm" id="updateStudentBtn">
    <span class="btn-text"><i class="fas fa-save mr-1"></i> Update Student</span>
</button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="excelExportForm" method="POST">
                    ...
                    <div class="form-group">
                        <label for="excelGenderFilter">Filter by Gender</label>
                        <select class="form-control" id="excelGenderFilter" name="gender">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="barcodeExportForm" method="POST">
                    ...
                    <!-- Added gender filter -->
                    <div class="form-group">
                        <label for="barcodeGenderFilter">Filter by Gender</label>
                        <select class="form-control" id="barcodeGenderFilter" name="gender">
                            <option value="">All Genders</option>
                            <option value="Male">Male</option>
                            <option value="Female">Female</option>
                        </select>
...
<script type="text/javascript" src="https://cdn.datatables.net/1.11.3/js/dataTables.bootstrap4.min.js"></script>
   ...
   // Edit button handler - using event delegation on the table
$('#activeStudentTable').on('click', '.edit-btn', function() {
...
// Barcode button handler - using event delegation on the table
$('#activeStudentTable').on('click', '.barcode-btn', function() {
    const lrn = $(this).data('lrn');
    const name = $(this).data('name');
...
// Archive button handler - using event delegation on the table
$('#activeStudentTable').on('click', '.archive-btn', function() {
    const studentId = $(this).data('id');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `My_Students.php?archive=${studentId}&status=Archived`;
        }
    });
});

// Restore button handler - using event delegation on the archived table
$('#archivedStudentTable').on('click', '.restore-btn', function() {
    const studentId = $(this).data('id');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `My_Students.php?archive=${studentId}&status=Active`;
        }
    });
});

// Delete button handler - using event delegation on the archived table
$('#archivedStudentTable').on('click', '.delete-btn', function() {
    const studentId = $(this).data('id');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `My_Students.php?delete=${studentId}`;
        }
    });
});
...
$('#printBarcodeBtn').click(function() {
    const printContent = `
        <div class="barcode-card">
            <h5 id="barcodeStudentName" class="mb-3">${$('#barcodeStudentName').text()}</h5>
            <div id="individualBarcodePreview" class="barcode-preview">
                ${$('#individualBarcodePreview').html()}
    ...
    e.preventDefault();
    const form = $(this);
    const gender = form.find('#excelGenderFilter').val();
...
$('#barcodeExportForm').submit(function(e) {
    e.preventDefault();
    const form = $(this);
    const gender = form.find('#barcodeGenderFilter').val();
   ...
   function generateBarcode(lrn, selector) {
    ...
    if (lrn && lrn.length >= 12) {
        ...
        svg.setAttribute('jsbarcode-width', '1.9');
...
$('#studentForm').on('submit', function() {
    ...
    $('#saveStudentBtn .btn-text').hide();
    $('#saveStudentBtn').append('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
});

$('#editStudentForm').on('submit', function() {
    $('#updateStudentBtn').prop('disabled', true);
    $('#updateStudentBtn .btn-text').hide();
    $('#updateStudentBtn').append('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Updating...');
});
</script>
